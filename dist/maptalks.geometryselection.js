...
var uid = 'geometryselection@cXiaof';
var options = {
    layers: [],
    availTypes: '*',
    colorHit: '#ffa400',
...
var GeometrySelection = function (_maptalks$Eventable) {
    ...
    GeometrySelection.prototype.addTo = function addTo(map) {
        if (!map) return this;
    ...
    GeometrySelection.prototype.getMap = function getMap() {
        return this._map;
    };
    ...
    GeometrySelection.prototype.forEach = function forEach() {
        ...
        this._geometries.forEach(function (geo, i, arr) {
            if (context) fn.call(context, geo, i, arr);else fn(geo, i, arr);
        });
        return this;
    };

    GeometrySelection.prototype.filter = function filter() {
        var fn = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : function () {
            return false;
        };
        var context = arguments[1];

        var isFn = this._isFn(fn);
        var filter = isFn ? fn : createFilter(fn);
        return this._geometries.filter(function (geo, i, arr) {
            geo = isFn ? geo : getFilterFeature(geo);
            return context ? filter.call(context, geo, i, arr) : filter(geo, i, arr);
        });
    };

    GeometrySelection.prototype.clear = function clear() {
    ...
    GeometrySelection.prototype._mousemoveEvents = function _mousemoveEvents(e) {
        ...
        target.identify({ coordinate: coordinate, tolerance: tolerance, layers: layers }, function (geos) {
            var hit = geos.reduce(function (target, geo) {
                if (target) return target;
                if (_this2._isAvailTypes(geo)) return geo;
                return target;
            }, null);
            _this2.hitGeo = hit;
            _this2._showCopyHitGeo();
        });
    };

    GeometrySelection.prototype._isAvailTypes = function _isAvailTypes(geo) {
        var availTypes = this.options['availTypes'];
    ...
    GeometrySelection.prototype._clickEvents = function _clickEvents() {
        var _this3 = this;

        if (this.hitGeo) {
            var coordHit = this._getSafeCoords(this.hitGeo);
            var chooseNext = this._geometries.reduce(function (chooses, geo) {
                var coord = _this3._getSafeCoords(geo);
                if (lodash_isequal(coordHit, coord)) return chooses;
                return [].concat(chooses, [geo]);
            }, []);
            if (chooseNext.length === this._geometries.length) {
                this.fire('choose', this.hitGeo);
                var newGeo = this.hitGeo.copy();
    ...
    GeometrySelection.prototype._getSafeCoords = function _getSafeCoords(geo) {
        var _geo = geo,
            options = _geo.options;
